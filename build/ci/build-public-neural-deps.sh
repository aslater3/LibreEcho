...
ESPEAK_SOURCE="$DEPS_ROOT/espeak-ng-source"
...
[[ -d "$ESPEAK_SOURCE/.git" ]] || fail "pinned eSpeak NG source checkout is missing: $ESPEAK_SOURCE"
...
# Sherpa's Piper phonemizer needs compiled eSpeak data, not only the source
# checkout's lang/ and voices/ inputs. Build this host-native data once into
# the neural cache; the generated files are architecture-independent.
ESPEAK_BUILD="$WORK/espeak-ng"
ESPEAK_DATA="$OUT/espeak-ng-data"
cmake -S "$ESPEAK_SOURCE" -B "$ESPEAK_BUILD" \
  -DCMAKE_BUILD_TYPE=MinSizeRel \
  -DENABLE_TESTS=OFF \
  -DUSE_LIBSONIC=OFF \
  -DUSE_MBROLA=OFF \
  -DUSE_SPEECHPLAYER=OFF
cmake --build "$ESPEAK_BUILD" --target data --parallel "$JOBS"
[[ -f "$ESPEAK_BUILD/espeak-ng-data/phontab" &&
   -f "$ESPEAK_BUILD/espeak-ng-data/phonindex" ]] || \
  fail "compiled eSpeak data is missing phontab/phonindex"
mkdir -p "$ESPEAK_DATA"
cp -a "$ESPEAK_BUILD/espeak-ng-data/." "$ESPEAK_DATA/"
[[ -f "$ESPEAK_DATA/phontab" && -f "$ESPEAK_DATA/phonindex" ]] || \
  fail "cached eSpeak data staging failed"
