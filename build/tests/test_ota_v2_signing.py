...
class OtaV2InputTests(unittest.TestCase):
    ...
    def test_v2_requires_the_actual_01314_candidate_and_pinned_prior_catalog(self) -> None:
        with self.assertRaisesRegex(InputError, "0.13.14"):
            validate_inputs(
                "v2", "0.14.0",
        ...
        with self.assertRaises(InputError):
            validate_inputs(
                ...
                "workflow_dispatch", "refs/heads/release/0.13.14",
            )
    ...
    def test_01314_accepts_only_the_pinned_01313_catalog(self) -> None:
        ...
        accepted = validate_inputs("v2", "0.13.14", url, "a" * 64,
                                   "workflow_dispatch", "refs/heads/release/0.13.14")
        self.assertEqual(accepted["ota_release"], "0.13.14")
        for stale in ("0.13.12", "0.13.14"):
            with self.assertRaises(InputError):
                validate_inputs("v2", "0.13.14", url.replace("0.13.13", stale),
                                "a" * 64, "workflow_dispatch", "refs/heads/release/0.13.14")
    ...
    def test_v2_publisher_gate_rejects_an_extra_capsule(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ota-v2-publisher-gate-") as directory:
            ...
            expected = [
                {"name": "libreecho-radar-puffin-0.13.14-assistant.runtime.squashfs"},
                {"name": "libreecho-radar-puffin-0.13.14-assistant.runtime-manifest.json"},
            ]
            for item in expected:
                (output / item["name"]).write_bytes(b"fixture")
            (output / "libreecho-radar-puffin-0.13.14-extra.payload.squashfs").write_bytes(b"extra")
            with self.assertRaisesRegex(ContractError, "publisher asset set"):
                validate_v2_publisher_asset_set(output, "0.13.14", expected)
...
class OtaV2SigningHandoffTests(unittest.TestCase):
    ...
    def test_handoff_schema_and_asset_hashes_are_rechecked(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            data = {
                ...
                "release": "0.13.14", "source_commit": "a" * 40,
            ...
            with self.assertRaisesRegex(HandoffError, "schema mismatch"):
                validate_handoff(handoff, "0.13.14")
    ...
    def test_actual_v2_platform_signer_consumes_handoff_and_product_rechecks_bundle(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="ota-v2-signer-fixture-") as directory:
            ...
            add_v2_contract(run, commits, release="0.13.14")
            ...
            with mock.patch.dict(os.environ, {"LIBREECHO_OTA_EXPECTED_PUBLIC_KEY_SHA256": anchor}):
                create_handoff(
                    run, "0.13.14", commits["ui"], "stable", base,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch("sign_ota_candidate.subprocess.run") as run:
                command = invoke_platform_signer(
                    ...
                    output=output, plan=plan, release="0.13.14",
