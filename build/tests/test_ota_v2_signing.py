...
class OtaV2InputTests(unittest.TestCase):
    ...
    def test_v2_requires_the_actual_01311_candidate_and_pinned_catalog(self) -> None:
        with self.assertRaisesRegex(InputError, "0.13.11"):
            validate_inputs(
                "v2", "0.14.0",
                "https://github.com/aslater3/LibreEcho/releases/download/radar-puffin-v0.13.10/libreecho-radar-puffin-v0.13.10-feature-catalog.json",
                "a" * 64, "workflow_dispatch", "refs/heads/release/0.13.11",
            )
        with self.assertRaises(InputError):
            validate_inputs(
                "v2", "0.13.11",
                "https://attacker.invalid/$(touch-pwned).json", "a" * 64,
                "workflow_dispatch", "refs/heads/release/0.13.11",
            )
    ...
    def test_v2_publisher_gate_rejects_an_extra_capsule(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ota-v2-publisher-gate-") as directory:
            ...
            expected = [
                {"name": "libreecho-radar-puffin-0.13.11-assistant.runtime.squashfs"},
                {"name": "libreecho-radar-puffin-0.13.11-assistant.runtime-manifest.json"},
            ]
            for item in expected:
                (output / item["name"]).write_bytes(b"fixture")
            (output / "libreecho-radar-puffin-0.13.11-extra.payload.squashfs").write_bytes(b"extra")
            with self.assertRaisesRegex(ContractError, "publisher asset set"):
                validate_v2_publisher_asset_set(output, "0.13.11", expected)
    ...
    def test_handoff_schema_and_asset_hashes_are_rechecked(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            data = {
                ...
                "release": "0.13.11", "source_commit": "a" * 40,
            ...
            with self.assertRaisesRegex(HandoffError, "schema mismatch"):
                validate_handoff(handoff, "0.13.11")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch("sign_ota_candidate.subprocess.run") as run:
                command = invoke_platform_signer(
                    ...
                    output=output, plan=plan, release="0.13.11",
