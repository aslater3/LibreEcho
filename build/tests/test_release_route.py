...
class ReleaseRouteTests(unittest.TestCase):
    ...
    def test_release_push_is_validation_only(self):
        self.assertEqual(route(self.request(), 'release/0.13.11', 'push'), 'none')
    ...
    def test_stable_retains_required_fields(self):
        ...
        request.update(version='0.13.11', release_tag='radar-puffin-v0.13.11', release_notes='release/notes.md', amonet_repository='reviewed', amonet_tag='reviewed', amonet_commit='a'*40, ssh_enabled='0')
        self.assertEqual(route(request, 'release/0.13.11', 'workflow_dispatch'), 'stable')
        for branch, event in [('main','workflow_dispatch'),('release/0.13.11','push'),('release/0.13.12','workflow_dispatch')]:
            with self.assertRaises(ValueError):
