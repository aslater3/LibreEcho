...
class ReleaseRouteTests(unittest.TestCase):
    ...
    def test_manual_release_dev_is_not_stable(self):
        self.assertEqual(route(self.request(), 'release/0.13.16', 'workflow_dispatch'), 'dev')

    def test_release_push_is_validation_only(self):
        self.assertEqual(route(self.request(), 'release/0.13.16', 'push'), 'none')
    ...
    def test_stable_retains_required_fields(self):
        ...
        with self.assertRaises(ValueError):
            route(request, 'release/0.13.16', 'workflow_dispatch')
        request.update(version='0.13.16', release_tag='radar-puffin-v0.13.16', release_notes='release/notes.md', amonet_repository='reviewed', amonet_tag='reviewed', amonet_commit='a'*40, ssh_enabled='0')
        self.assertEqual(route(request, 'release/0.13.16', 'workflow_dispatch'), 'stable')
        for branch, event in [('main','workflow_dispatch'),('release/0.13.16','push'),('release/0.13.13','workflow_dispatch')]:
            with self.assertRaises(ValueError):
                route(request, branch, event)

    def test_invalid_request_rejected(self):
        for request in (None, {}, self.request('unknown'), self.request() | {'version':'0.13.16'}):
            with self.assertRaises(ValueError):
