...
initial deployment target is a separately named persistent VM on `nixtop`.
...
- `LibreEcho-platform` owns release-image extraction, VM/container assembly,
  persistent-nixtop lifecycle scripts, service startup, port allocation,
...
suite on `nixtop`, not a required fast presubmit check.
...
- The VM binds the control interface to loopback by default. Remote access on
  `nixtop` occurs through an explicit SSH tunnel or equivalently restricted
  transport.
...
rapid repeats so debounce behavior can be tested. Events enter the same
button-daemon path used by physical input.

### Virtual Microphone

The virtual microphone accepts mono signed 16-bit PCM at 16 kHz as its canonical
format. WAV input is validated and converted before streaming. Injection uses
timed chunks and configurable gain rather than writing the entire utterance at
once. Scenarios may insert silence, split the wake word and command across
chunks, add versioned background-noise fixtures, or mix competing speech.
...
matrix for a critical subset. The persistent `nixtop` acceptance run may
...
real-model run on `nixtop`.

## Persistent Nixtop VM
...
Artifacts are retained on failure and exportable from `nixtop`. Successful
...
4. Real-model `nixtop` acceptance scenarios exercise wake, STT, assistant, TTS,
...
- the persistent isolated `nixtop` VM can be built and started from committed
  repository scripts;
...
- at least one real-model wake-and-stop run passes on `nixtop`;
