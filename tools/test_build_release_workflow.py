...
class Tests(unittest.TestCase):
 ...
 def test_triggers_and_jobs(self):
  self.assertIn("branches: [main, 'release/**']",W); self.assertIn("cron: '17 3 * * *'",W); self.assertIn('workflow_dispatch:',W); self.assertIn('update_channel:',W); self.assertIn('release_version:',W); self.assertIn('ssh_enabled:',W); self.assertIn('options: [disabled, enabled]',W); self.assertIn('SSH_ENABLED_INPUT',W); self.assertNotIn('inputs.version',W)
 ...
 def test_release_lanes_and_ota_boundaries(self):
  ...
  self.assertIn("local) [[ \"$channel\" == stable ]]", PUBLIC_WRAPPER)
